...
inner_app_code <- paste(
  c(
    ...
    "}",
    "",
    "choose_display_unit <- function(values, internal_unit) {",
    "  values <- values[is.finite(values) & !is.na(values)]",
    "",
    "  if (length(values) == 0 || is.na(internal_unit)) return(internal_unit)",
    "",
    "  median_abs <- stats::median(abs(values[values != 0]), na.rm = TRUE)",
    "",
    "  if (!is.finite(median_abs)) return(internal_unit)",
    "",
    "  case_when(",
    "    internal_unit == \"mg\" & median_abs < 1 ~ \"mikrogram\",",
    "    internal_unit == \"mg\" & median_abs >= 10000 ~ \"g\",",
    "    TRUE ~ internal_unit",
    "  )",
    "}",
    "",
    "convert_from_internal_to_display <- function(value, internal_unit, display_unit) {",
    "  case_when(",
    "    is.na(value) ~ NA_real_,",
    "    internal_unit == \"mg\" & display_unit == \"mikrogram\" ~ value * 1000,",
    "    internal_unit == \"mg\" & display_unit == \"g\" ~ value / 1000,",
    "    TRUE ~ value",
    "  )",
    "}",
    "",
    "round_display_amount <- function(value, display_unit) {",
    "  case_when(",
    "    is.na(value) ~ NA_real_,",
    "    display_unit == \"mikrogram\" ~ round(value, 1),",
    "    display_unit == \"g\" ~ round(value, 3),",
    "    TRUE ~ round(value, 1)",
    "  )",
    "}",
    "",
    ...
    "        \"Mikrogram og gram omregnes automatisk til mg ved intern beregning, så mængder kan summeres korrekt. I tabeller og grafer vises værdier automatisk i den mest læsbare enhed, fx mikrogram ved meget små mængder og gram ved meget store mængder (fra 10.000 mg og opefter). Andre enheder som fx enheder, IE, mol og mmol bevares. Procentstyrker beregnes ud fra pakningsmængden, fx 0,1% af 30 g = 30 mg og 0,1% af 90 ml = 90 mg (antaget 1 g/ml). Hvis samme ATC-kode findes med flere forskellige interne enheder, vises de separat. Kombinationspræparater med '+' medregnes kun med første dosisangivelse. Pakninger med 'x' tolkes multiplikativt, fx '5 x 3 ml' = 15 ml.\"",
    "      ),",
    "      uiOutput(\"warnings\"),",
    "      uiOutput(\"table\"),",
    ...
    "            dose_kind == \"percent\" & pack_unit_base %in% c(\"mg\", \"ml\") ~ \"mg\",",
    "            dose_kind == \"percent\" ~ pack_unit_base,",
    "            TRUE ~ dose_unit_base",
    "          ),",
    "          row_total_amount = case_when(",
    "            dose_kind == \"percent\" & pack_unit_base == \"mg\" & !is.na(strength_fraction) & !is.na(pack_amount_base) & !is.na(Antal.pakker_num) ~ strength_fraction * pack_amount_base * Antal.pakker_num,",
    "            dose_kind == \"percent\" & pack_unit_base == \"ml\" & !is.na(strength_fraction) & !is.na(pack_amount_base) & !is.na(Antal.pakker_num) ~ strength_fraction * pack_amount_base * 1000 * Antal.pakker_num,",
    ...
    "        group_by(ATC, intern_enhed = result_unit, Dato) %>%",
    "        summarize(",
    "          Lægemiddel = dplyr::last(Lægemiddel),",
    "          total_maengde_intern = sum(row_total_amount, na.rm = TRUE),",
    "          antal_udeladte_linjer = sum(is.na(row_total_amount)),",
    "          .groups = \"drop\"",
    "        ) %>%",
    "        arrange(ATC, intern_enhed, Dato) %>%",
    "        group_by(ATC, intern_enhed) %>%",
    "        mutate(",
    "          periode_dage = abs(as.numeric(difftime(Dato, lead(Dato), units = \"days\"))),",
    "          dagligt_forbrug_intern = if_else(",
    "            !is.na(periode_dage) & periode_dage > 0,",
    "            total_maengde_intern / periode_dage,",
    "            NA_real_",
    "          ),",
    "          rullende_gennemsnit_intern = zoo::rollmean(",
    "            dagligt_forbrug_intern,",
    "            k = 3,",
    "            fill = NA,",
    "            align = \"right\"",
    "          ),",
    "          total_dosage = sum(total_maengde_intern[Dato != max(Dato)], na.rm = TRUE),",
    "          total_days = as.numeric(max(Dato) - min(Dato)),",
    "          gennemsnit_hele_perioden_intern = if_else(",
    "            total_days > 0,",
    "            total_dosage / total_days,",
    "            NA_real_",
    "          ),",
    "          display_unit = choose_display_unit(",
    "            c(dagligt_forbrug_intern, rullende_gennemsnit_intern, gennemsnit_hele_perioden_intern),",
    "            dplyr::first(intern_enhed)",
    "          ),",
    "          `Total mængde` = round_display_amount(",
    "            convert_from_internal_to_display(total_maengde_intern, intern_enhed, display_unit),",
    "            display_unit",
    "          ),",
    "          `Dagligt forbrug (gennemsnit for aktuelle periode)` = round_display_amount(",
    "            convert_from_internal_to_display(dagligt_forbrug_intern, intern_enhed, display_unit),",
    "            display_unit",
    "          ),",
    "          `Rullende gennemsnit (sidste 3 perioder)` = round_display_amount(",
    "            convert_from_internal_to_display(rullende_gennemsnit_intern, intern_enhed, display_unit),",
    "            display_unit",
    "          ),",
    "          `Gennemsnit for hele perioden` = round_display_amount(",
    "            convert_from_internal_to_display(gennemsnit_hele_perioden_intern, intern_enhed, display_unit),",
    "            display_unit",
    "          ),",
    "          Enhed = display_unit,",
    ...
    "          sprintf(",
    "            \"%s præparat(er) var angivet i mikrogram og er omregnet til mg ved intern beregning. Små værdier vises automatisk i mikrogram i tabeller og grafer.\",",
...
ui <- secure_app(
  fluidPage(
    titlePanel("Kør compliance-appen (version 3.1) lokalt på egen computer"),
    
    h3("Vejledning:"),
    p("1) Installér R og RStudio via link nedenfor (husk at aktivere Heimdal Agent på AU-computere). På Region Nord-computere kan R og RStudio installeres via appen IT-Support under selvbetjening."),
    ...
    p("6) Kør scriptet for at åbne appen i et nyt vindue."),

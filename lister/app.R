...
get_acb_score <- function(atc_codes) {
  acb_subset <- acb_data[acb_data$ATC %in% atc_codes & !is.na(acb_data$ACB), ]
  total_acb <- sum(acb_subset$ACB, na.rm = TRUE)
  ...
  observeEvent(input$generate_anticholinergic, {
    ...
    output$acb_output <- renderUI({
      if (length(atc_codes()) > 0) {
        ...
        HTML(paste0(
          "<h4 style='font-size:16px;'><b>Antikolinerg belastning (kilde: IRF’s liste over antikolinerge lægemidler, 2019):</b></h4>",
  ...
  observeEvent(input$generate_seponeringslisten, {
    ...
    output$seponeringslisten_output <- renderUI({
      if (!is.null(seponerings_data()) && nrow(seponerings_data()) > 0) {
        ...
        HTML(paste0(
          "<h4 style='font-size:16px;'><b>Seponeringslisten 2026 (kilde: Medicinrådet):</b></h4>",

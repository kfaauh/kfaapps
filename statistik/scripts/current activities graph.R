...
p_backlog <- ggplot() +
  ...
  scale_fill_manual(
    values = vibrant_palette[svar_levels_plot],
  ...
  labs(x = NULL, y = "Forespørgsler", title = NULL, subtitle = NULL) +
...
p_backlog_noleg <- p_backlog + theme(
...
# -----------------------------------------------------------------------------
# Conditional status legend entry for "Sendt (dato mangler)"
# -----------------------------------------------------------------------------

missing_status <- "Sendt (dato mangler)"
base_status_levels <- c("Modtaget", "Fordelt", "Startet", "Ved bagvagt")

has_missing_status <- activity_plot_data %>%
  filter(is.na(FaerdigDate) | FaerdigDate >= as_of) %>%
  mutate(age_adjusted = as.integer(as_of - AdjustedDate)) %>%
  filter(dplyr::between(age_adjusted, 0, 44)) %>%
  summarise(any_missing = any(Status == missing_status, na.rm = TRUE)) %>%
  pull(any_missing)

status_levels <- if (has_missing_status) c(base_status_levels, missing_status) else base_status_levels
...
if (has_missing_status) {
  status_cols[missing_status] <- "#2E8B57"
}

filled_statuses <- c("Fordelt", "Startet", "Ved bagvagt")
if (has_missing_status) {
  filled_statuses <- c(filled_statuses, missing_status)
}

legend_shapes <- c(13, rep(21, length(status_levels) - 1))
legend_fills <- c(NA, unname(status_cols[status_levels[-1]]))
...
scatter_df <- activity_plot_data %>%
  ...
  mutate(
    ...
    svar_kategori_plot = if_else(svar_kategori == "Kortsvar", "Almindeligt svar", svar_kategori),
    Status = case_when(
      Status %in% status_levels ~ Status,
      TRUE ~ NA_character_
    ),
    x_age = if_else(age_adjusted <= 21, age_adjusted, 23L)
  ) %>%
  filter(
    !is.na(svar_kategori_plot),
    !is.na(Status),
    dplyr::between(age_adjusted, 0, 44)
  ) %>%
  mutate(
    svar_kategori_plot = factor(svar_kategori_plot, levels = y_levels),
    Status = factor(Status, levels = status_levels)
  ) %>%
  add_count(x_age, svar_kategori_plot, name = "n_grp") %>%
  group_by(x_age, svar_kategori_plot) %>%
  mutate(
    idx_in_grp = row_number(),
    y_base   = as.integer(svar_kategori_plot),
    y_offset = ifelse(n_grp == 1, 0, scales::rescale(idx_in_grp, to = c(-0.2, 0.2))),
...
p_age_bottom <- ggplot() +
  geom_point(
    ...
    aes(x = x_age, y = y_idx, fill = Status),
    shape = 21,
    size = 2.8,
    stroke = 0.6,
    colour = "black",
  ...
  geom_point(
    data = scatter_df %>% filter(Status %in% filled_statuses),
    aes(x = x_age, y = y_jit, fill = Status),
    shape = 21,
    size = 2.8,
    stroke = 0.6,
    colour = "black",
  ...
  geom_point(
    data = scatter_df %>% filter(Status == "Modtaget"),
    aes(x = x_age, y = y_jit),
    shape = 13,
    size = 2.8,
    colour = "black",
...
if (length(missing_names) > 0) {
  missing_names_text <- if (length(missing_names) > 5) {
    paste0(paste(missing_names[1:5], collapse = ", "), ", ...")
  } else {
...
if (nrow(conflicting_pairs) > 0) {
  ...
  pair_text <- if (length(pair_labels) > 5) {
    paste0(paste(pair_labels[1:5], collapse = ", "), ", ...")
  } else {

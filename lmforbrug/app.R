...
# Select/deselect observers
  ...
  # Reactive plot with separate aes branches
...
# Ensure each (Dato, Produktnavn, SubstGruppe, Område) is only one row
  df_unique <- df %>%
    group_by(Dato, Produktnavn, SubstGruppe, Område) %>%
    summarize(y = first(.data[[y_var]]), .groups = "drop")

# 3. now do your stratified or un-stratified summarisation
